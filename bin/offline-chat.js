...
async function checkOllamaSetup() {
    ...
    try {
        ...
        const hasLlama = response.models?.some((model) => model.name.includes('llama'));
...
class OfflineDatabaseChatService {
    constructor(searchRepo, crudRepo, ollamaUrl = 'http://localhost:11434', chatModel = 'llama3.2:3b' // Fast 3B model, good for chat
    ) {
